...
export default function EditInformation() {
  ...
  useEffect(() => {
    const fetchDocumentData = async (id: string) => {
      try {
        ...
        if (snapshot.exists()) {
          ...
          setDocumentId(docData.awdReferenceNumber || id);
        } else {
          ...
          if (trackSnapshot.exists()) {
            ...
            setDocumentId(trackData.awdReferenceNumber || id);
          } else {
    ...
    if (finalAwdRefnu) {
      ...
      fetchDocumentData(finalAwdRefnu);
    } else if (storedAwdRef) {
      setDocumentId(storedAwdRef);
      fetchDocumentData(storedAwdRef);
    }
  }, [params, router]);
